...
- `apps/web/src/pages/ChatPage.tsx` (integrato useSessionHistory hook, loading indicator, graceful degradation, merge history messages, lazy loading pagination con scroll listener, UI indicator load more, merge incrementale paginato; **Fix 1.14.1**: lazy initialization sessionId `useState(() => getOrCreateSessionId())`)
